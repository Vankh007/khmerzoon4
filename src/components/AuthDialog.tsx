...
export function AuthDialog({ open, onOpenChange, onSuccess, container }: AuthDialogProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent container={container} className="max-w-[95vw] sm:max-w-md landscape:max-w-[50vw] landscape:md:max-w-[40vw] landscape:max-h-[90vh] max-h-[85vh] overflow-y-auto p-4 sm:p-6 landscape:p-3 landscape:py-4 bg-transparent border-2 border-primary backdrop-blur-none shadow-none">
        <DialogHeader className="landscape:pb-2 pb-3">
          <div className="flex justify-center mb-2 sm:mb-3 landscape:mb-1.5">
            <img 
              src={logo} 
              alt="KHMERZOON" 
              className="w-12 h-12 sm:w-16 sm:h-16 landscape:w-10 landscape:h-10 object-contain"
            />
          </div>
          <DialogTitle className="text-center text-xl sm:text-2xl landscape:text-lg font-bold">
            {isSignUp ? 'Create Account' : 'Sign In'}
          </DialogTitle>
          <DialogDescription className="text-center text-sm landscape:text-xs">
        ...
        <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4 landscape:space-y-2 mt-2 sm:mt-4 landscape:mt-2">
          ...
          {isSignUp && (
            <div className="flex items-start space-x-2 p-3 landscape:p-2 bg-muted/30 rounded-lg">
              <button
                type="button"
                onClick={() => setAgreedToTerms(!agreedToTerms)}
                className="mt-0.5 flex-shrink-0"
              >
                <div className={`w-5 h-5 landscape:w-4 landscape:h-4 rounded border-2 flex items-center justify-center transition-colors ${
                  agreedToTerms 
                    ? 'bg-primary border-primary' 
                    : 'border-muted-foreground/50'
                }`}>
                  {agreedToTerms && <CheckCircle2 className="w-3.5 h-3.5 landscape:w-3 landscape:h-3 text-primary-foreground" />}
                </div>
              </button>
              <label 
                className="text-sm landscape:text-xs text-muted-foreground cursor-pointer select-none"
                onClick={() => setAgreedToTerms(!agreedToTerms)}
              >
          ...
          <Button 
            type="submit" 
            className="w-full h-11 sm:h-12 landscape:h-9 text-base landscape:text-sm font-semibold" 
            disabled={loading || (isSignUp && !agreedToTerms)}
          >
            {loading ? 'Loading...' : isSignUp ? 'Sign Up' : 'Sign In'}
          </Button>

          <div className="relative my-3 sm:my-4 landscape:my-2">
            <div className="absolute inset-0 flex items-center">
              <span className="w-full border-t" />
            </div>
            <div className="relative flex justify-center text-xs landscape:text-[11px] uppercase">
          ...
          <Button 
            type="button" 
            variant="outline" 
            className="w-full h-11 sm:h-12 landscape:h-9 text-base landscape:text-sm font-medium" 
            onClick={handleGoogleSignIn}
            disabled={loading}
          >
            <svg className="mr-2 h-5 w-5 landscape:h-4 landscape:w-4" viewBox="0 0 24 24">
              <path
                fill="currentColor"
          ...
          <div className="text-center pt-1">
            <button
              ...
              className="text-sm landscape:text-xs text-primary hover:underline font-medium"
            >
