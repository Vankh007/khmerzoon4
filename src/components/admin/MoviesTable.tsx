...
export function MoviesTable() {
  ...
  const { data: movies, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data?.filter((movie: any) => {
        const upcomingRelease = movie.upcoming_releases?.[0];
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: (_, { status }) => {
      toast.success(`${selectedIds.length} movies set to ${status}`);
      queryClient.invalidateQueries({ queryKey: ['admin-movies'] });
      setSelectedIds([]);
    },
    onError: (error: Error) => {
  ...
  const versionMutation = useMutation({
    ...
    onSuccess: (_, { access_type }) => {
      toast.success(`${selectedIds.length} movies set to ${access_type}`);
      queryClient.invalidateQueries({ queryKey: ['admin-movies'] });
      setSelectedIds([]);
    },
    onError: (error: Error) => {
  ...
  const handleSelectAll = (checked: boolean) => {
    setSelectedIds(checked ? (filteredMovies?.map(m => m.id) || []) : []);
  };

  const handleSelectOne = (id: string, checked: boolean) => {
    setSelectedIds(checked ? [...selectedIds, id] : selectedIds.filter(mid => mid !== id));
  };
                ...
                <TableRow key={movie.id}>
                  ...
                  <TableCell>
                    <select 
                      ...
                      onChange={(e) => {
                        statusMutation.mutate({ ids: [movie.id], status: e.target.value });
                      }}
                    >
