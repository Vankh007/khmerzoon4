...
const AppLockOverlay: React.FC<AppLockOverlayProps> = ({ type, contentBackdrop, onClose }) => {
  ...
  // Get primary color for styling (icons and text) from theme colors
  const primaryColor = theme === 'dark' 
    ? (settings.dark_mode?.primary_color || '#00bcd4')
    : (settings.light_mode?.primary_color || '#00bcd4');
  ...
  // Desktop layout - horizontal with QR code visible
  return (
    <div 
    ...
    >
      ...
      {onClose && (
        <Button
          ...
          size="icon"
          onClick={onClose}
          className="absolute top-4 right-4 z-20 h-10 w-10 text-white/70 hover:text-white hover:bg-white/10 rounded-full"
        >
          <X className="h-5 w-5" />
        </Button>
      )}
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center justify-center gap-6 p-6 w-auto mx-auto animate-scale-in">
        ...
        <div className="flex flex-col items-center md:items-start gap-5 flex-1">
          {/* Logo and Site Name with primary color */}
          <div className="flex items-center gap-3">
            {currentLogo ? (
              <img 
                src={currentLogo} 
                alt={settings.site_title}
                className="w-14 h-14 object-contain rounded-lg p-1.5"
        ...
        {/* Right side - QR Code (visible on all screens) */}
        {qrUrl && (
