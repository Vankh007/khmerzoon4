...
interface MovieCardProps {
  ...
  category?: string;
  cast?: CastMember[];
  castNames?: string; // Comma-separated cast names from database
...
const MovieCard = ({ id, title, description, imageUrl, category, cast, castNames, contentType = 'movie', accessType, recentEpisode, onClick }: MovieCardProps) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Cast Members - show images if available, otherwise show names */}
        {castArray.length > 0 ? (
          <div className="pt-3 border-t border-white/20 mt-2">
            ...
            <div className="flex items-center gap-2">
              {castArray.slice(0, 5).map((member) => (
                <div
                  key={member.id}
                  className="relative w-10 h-10 rounded-full border-2 border-white/40 overflow-hidden flex-shrink-0"
                ...
                >
                  {member.profile_path ? (
                    <img
                      ...
                      onError={(e) => {
                        ...
                        if (parent) {
                          ...
                          fallback.style.backgroundColor = 'hsl(var(--primary) / 0.3)';
                    ...
                    <div className="w-full h-full flex items-center justify-center text-lg" style={{ backgroundColor: 'hsl(var(--primary) / 0.3)' }}>
                      👤
                    </div>
                  )}
                </div>
        ...
        ) : castNames && castNames.trim() ? (
          <div className="pt-3 border-t border-white/20 mt-2">
            <p className="text-[11px] font-semibold text-white/80 mb-2">Cast</p>
            <p className="text-xs text-white/70 line-clamp-2">{castNames}</p>
          </div>
        ) : null}
        </div>
      </div>
