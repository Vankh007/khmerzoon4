...
export function useNativeShakaPlayer({
  ...
  const loadSource = useCallback(async (url: string, mimeType?: string) => {
    ...
    // Auto-detect source type (keep outside try so catch can use it for fallback)
    ...
    try {
      ...
      // Load new source with retry logic
      const loadStartTime = Date.now();
      let loadAttempt = 0;
      const maxAttempts = detectedType === 'mp4' ? 2 : (platform === 'android' ? 3 : 1);
      ...
      while (loadAttempt < maxAttempts) {
        try {
          ...
          if (loadAttempt < maxAttempts) {
            logDebug('loadSource', `Load failed, retrying in 300ms...`, err.message);
      ...
      // For MP4, set up for progressive playback
      if (detectedType === 'mp4' && videoRef.current) {
        // Enable smooth playback optimizations
        videoRef.current.preload = 'auto';
      ...
      // On Android or for MP4, try to start playback after a short delay if video is ready
      if ((platform === 'android' || detectedType === 'mp4') && videoRef.current) {
        setTimeout(() => {
          if (videoRef.current && videoRef.current.readyState >= 2) {
            logDebug('loadSource', 'Video ready, can play');
          }
        }, 200);
      }
      ...
      // Fallback: On Safari/iOS, some HLS streams can play natively even if Shaka fails.
      // This won't bypass CORS, but it helps when the failure is MSE/compat-related.
      if (detectedType === 'hls' && videoRef.current) {
        ...
        if (canNativeHls) {
          try {
            logDebug('loadSource', 'Attempting native HLS fallback');
            // Detach Shaka to avoid conflicts, then play using native pipeline.
            ...
            logDebug('loadSource', 'Native HLS fallback failed:', fallbackErr);
          }
        }
      }
  ...
  }, [initPlayer, isNative, platform, getNativeConfig, onQualitiesLoaded, onAudioTracksLoaded, onTextTracksLoaded, onLoaded, onError, videoRef]);
